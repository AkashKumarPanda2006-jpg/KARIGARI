...
| AI | Google **Gemini** (`@google/genai`) only — vision, valuation, translation, and speech-to-text. The voice assistant sends the recorded audio straight to Gemini, which transcribes and answers in one call. Degrades to a spoken apology if the key is absent |
...
graph TD
  ...
  DASH --> VO["VoiceOnboarding<br/>/api/voice-assistant (Gemini audio in, reply out)"]:::ai
