...
export async function POST(req: Request) {
  try {
    ...
    const { craftType, rawMaterialCost, laborDays, descriptionOriginal, descriptionEnglish, tags, assignedAdminId, images, aiGeneratedListing, askingPrice, dummyId } = body;
    ...
    if (dummyId) {
      await prisma.ivrDummyItem.update({
        where: { id: dummyId },
        data: { status: 'MIGRATED' }
      }).catch((err: any) => console.error("Failed to migrate IVR dummy:", err));
    }
