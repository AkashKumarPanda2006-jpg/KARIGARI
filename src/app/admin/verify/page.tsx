...
export default function VerifyBatchPage() {
  ...
  const fetchPendingItems = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Get non-pending (verified/dispatched) captures, ideally we'd filter for specifically ADVANCE_PAID or VERIFIED status, but recentCaptures works here.
        setVerifiedItems(data.data.recentCaptures || []);
      }
    } catch (e) {
  ...
  // Group items by artisan for Pending
  ...
  // Group items by artisan for Verified
  const groupedVerified = verifiedItems.reduce((acc, item) => {
    if (!acc[item.artisanId]) {
      acc[item.artisanId] = {
        artisanName: item.artisan?.name || 'Unknown Artisan',
        items: []
      };
    }
    acc[item.artisanId].items.push(item);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <main className="flex-grow p-6 sm:p-10 max-w-6xl mx-auto w-full space-y-12">
        <div>
        ...
        {/* VERIFIED BATCHES LOG */}
        {!isLoading && Object.keys(groupedVerified).length > 0 && (
          <div>
            <div className="flex items-center gap-3 mb-6 border-t border-gray-200 pt-8">
              ...
              <div>
                <h2 className="text-xl font-serif font-bold text-gray-900">Verification & Dispatch Log</h2>
                <p className="text-sm text-gray-500">History of recently approved batches.</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {Object.entries(groupedVerified).map(([artisanId, group]) => (
                <div key={artisanId} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden opacity-75 hover:opacity-100 transition-opacity">
                  <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <div>
                      <h3 className="font-bold text-gray-900">{group.artisanName}</h3>
                      <p className="text-xs text-gray-500">{group.items.length} items verified</p>
                    </div>
                    <div className="flex items-center gap-2 bg-green-100 text-green-700 px-3 py-1.5 rounded-lg text-xs font-bold border border-green-200">
                      <CheckCircle size={14} />
                  ...
                  <div className="divide-y divide-gray-50">
                    {group.items.map((item) => (
                      <div key={item.id} className="p-4 flex gap-4 items-center">
                        ...
                        <div className="flex-grow">
                          <h4 className="font-bold text-gray-900 text-sm">{item.craftType}</h4>
                          <p className="text-xs text-gray-500 truncate max-w-sm">{item.descriptionEnglish}</p>
                        </div>
                        <div className="text-right">
                          <span className="block text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-0.5">NFC Patch ID</span>
                          <span className="font-mono text-xs font-bold text-gray-700">{item.patchId || "PENDING"}</span>
                        </div>
                      </div>
                    ))}
                  </div>
