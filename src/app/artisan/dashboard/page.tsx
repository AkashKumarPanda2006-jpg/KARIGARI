...
import {
  ...
  LogOut, X, MapPin, Award, Camera, FileText, ArrowRightCircle, Clock, CheckCircle2, AlertTriangle, Bot
} from "lucide-react";
...
export default function ArtisanDashboard() {
  ...
  const [prefillDummy, setPrefillDummy] = useState<any>(null);
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans pb-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <div className="text-left relative z-10">
              ...
              <div className="text-sm text-white/80 leading-relaxed max-w-[85%]">
                {t('capture_subtitle')}
              </div>
            </div>
          </button>

          {/* Action 2 (Raw Materials) */}
          <Link href="/artisan/materials" className="rounded-3xl p-6 flex flex-col justify-between min-h-[160px] bg-blue-50 border border-blue-100 shadow-sm transition-all hover:-translate-y-1 hover:border-blue-200 hover:shadow-md group">
            <div>
              <div className="w-10 h-10 rounded-2xl bg-blue-100 text-blue-700 flex items-center justify-center mb-4">
                <Package size={20} />
              </div>
              <div className="text-lg font-bold text-gray-900 mb-1">{t('raw_materials_title')}</div>
              <div className="text-sm text-gray-600 leading-relaxed">
                {t('raw_materials_subtitle')}
              </div>
            </div>
          </Link>

          {/* Action 3 (Learning AI) */}
          <button
            onClick={() => setIsLearningModalOpen(true)}
            className="group block w-full text-left relative overflow-hidden bg-white rounded-3xl p-6 shadow-sm border border-gray-100 transition-all hover:shadow-md hover:border-purple-200"
          >
            <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
              <ArrowRightCircle className="text-purple-600" size={24} />
            </div>
            <div>
              <div className="w-10 h-10 rounded-2xl bg-purple-100 text-purple-700 flex items-center justify-center mb-4">
                <Bot size={20} />
              </div>
              <div className="text-lg font-bold text-gray-900 mb-1">{t('learn_ai_title')}</div>
              <div className="text-sm text-gray-600 leading-relaxed">
                {t('learn_ai_subtitle')}
              </div>
            </div>
          ...
          {/* Action 4 (Schemes) */}
          <Link href="/artisan/schemes" className="rounded-3xl p-6 flex flex-col justify-between min-h-[160px] bg-white border border-gray-200 shadow-sm transition-all hover:-translate-y-1 hover:border-gray-300 hover:shadow-md group">
            <div>
              <div className="w-10 h-10 rounded-2xl bg-green-50 text-[#24332C] flex items-center justify-center mb-4">
                <FileText size={20} />
              </div>
              <div className="text-lg font-bold text-gray-900 mb-1">{t('apply_for_schemes')}</div>
          ...
          {/* Action 5 (Insights) */}
          <Link href="/artisan/insights" className="rounded-3xl p-6 flex flex-col justify-between min-h-[160px] bg-white border border-gray-200 shadow-sm transition-all hover:-translate-y-1 hover:border-gray-300 hover:shadow-md group">
            <div>
              <div className="w-10 h-10 rounded-2xl bg-green-50 text-[#24332C] flex items-center justify-center mb-4">
                <Globe size={20} />
              </div>
              <div className="text-lg font-bold text-gray-900 mb-1">{t('global_insights')}</div>
              <div className="text-sm text-gray-500 leading-relaxed">
                {t('insights_subtitle')}
              </div>
            </div>
          ...
          {/* Action 6 (Live News) */}
          <Link href="/artisan/news" className="rounded-3xl p-6 flex flex-col justify-between min-h-[160px] bg-white border border-gray-200 shadow-sm transition-all hover:-translate-y-1 hover:border-green-300 hover:shadow-md group">
            <div>
              <div className="w-10 h-10 rounded-2xl bg-green-100 text-green-700 flex items-center justify-center mb-4">
                <Globe size={20} />
              </div>
              <div className="text-lg font-bold text-gray-900 mb-1">Live News & Events</div>
              <div className="text-sm text-gray-500 leading-relaxed">
                Get hyper-local updates on your craft.
              </div>
            </div>
          </Link>
        ...
        {/* PENDING IVR CAPTURES */}
        {dashboardData?.pendingIvrItems?.length > 0 && (
          <div className="mb-10">
            <div className="text-xs font-bold tracking-wider uppercase text-orange-500 mt-10 mb-4 px-2 flex items-center gap-2">
              <AlertTriangle size={14} /> Pending IVR Captures (Action Required)
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {dashboardData.pendingIvrItems.map((dummy: any) => (
                <div key={dummy.id} className="bg-orange-50/50 border border-orange-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="w-2 h-2 rounded-full bg-orange-500"></span>
                      <span className="text-xs font-bold text-orange-700 uppercase">Awaiting SHG Photo</span>
                    </div>
                    <h4 className="font-bold text-gray-900 mt-2">{dummy.craftType}</h4>
                    <p className="text-sm text-gray-600 mt-1 line-clamp-2 italic">"{dummy.transcript}"</p>
                    <div className="flex gap-4 mt-3 text-xs font-medium text-gray-500">
                      <span>{dummy.laborDays} Days</span>
                      <span>₹{dummy.rawMaterialCost} Mat.</span>
                      <span>{new Date(dummy.createdAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                  <button 
                    onClick={() => {
                      setPrefillDummy(dummy);
                      setIsModalOpen(true);
                    }}
                    className="mt-5 w-full py-2.5 bg-orange-100 hover:bg-orange-200 text-orange-800 font-bold rounded-xl transition-colors flex items-center justify-center gap-2"
                  >
                    <Camera size={16} /> Complete Capture
                  </button>
                </div>
              ))}
            </div>
      ...
      {/* MODALS */}
      <CaptureModal isOpen={isModalOpen} onClose={handleModalClose} prefillDummy={prefillDummy} />
      <AgentHandoffModal 
      ...
      />
      <LearningAssistantModal isOpen={isLearningModalOpen} onClose={() => setIsLearningModalOpen(false)} />
